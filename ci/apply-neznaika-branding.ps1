...
if (Test-Path $setupPath) {
    ...
    $setup = [regex]::Replace($setup, '"ProductVersion" = "8:[^"]+"', '"ProductVersion" = "8:1.0.18"')
...
Write-Host ''
Write-Host 'НеZнайка 1.0.18 — установка' -ForegroundColor Cyan
Write-Host '================================'
...
Write-Host ''
Write-Host 'НеZнайка 1.0.18 установлена.' -ForegroundColor Green
...
$readme = @'
НеZнайка 1.0.18 для Microsoft Word
=================================
...
Write-Host 'НеZнайка branding, version 1.0.18, owl resource and installer bundle prepared.'
