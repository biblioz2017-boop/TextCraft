...
if (Test-Path $assemblyPath) {
    ...
    $assembly = [regex]::Replace($assembly, 'AssemblyVersion\("[^"]+"\)', 'AssemblyVersion("1.0.17.0")')
    $assembly = [regex]::Replace($assembly, 'AssemblyFileVersion\("[^"]+"\)', 'AssemblyFileVersion("1.0.17.0")')
...
if (Test-Path $projectPath) {
    ...
    $project = [regex]::Replace($project, '<ApplicationVersion>[^<]*</ApplicationVersion>', '<ApplicationVersion>1.0.17.0</ApplicationVersion>')
...
if (Test-Path $setupPath) {
    ...
    $setup = [regex]::Replace($setup, '"ProductVersion" = "8:[^"]+"', '"ProductVersion" = "8:1.0.17"')
...
Write-Host ''
Write-Host 'НеZнайка 1.0.17 — установка' -ForegroundColor Cyan
Write-Host '================================'
...
Write-Host ''
Write-Host 'НеZнайка 1.0.17 установлена.' -ForegroundColor Green
...
$readme = @'
НеZнайка 1.0.17 для Microsoft Word
=================================
...
Write-Host 'НеZнайка branding, version 1.0.17, owl resource and installer bundle prepared.'
